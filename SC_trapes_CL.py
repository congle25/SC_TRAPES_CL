...
# Single-cell compatible version of TRAPeS to support parallel processing with each thread running on data from one cell (PE)
# By Le Cong (Regev Lab - Broad Institute)
def runTCRpipe(genome, output, bam, unmapped, bases, strand, numIterations,thresholdScore, minOverlap, rsem, bowtie2, singleCell, path, sumF, lowQ, samtools):
    ...
    if singleCell == True: # Enabled single-cell tag
        if path == './':
	    path = os.getcwd()
        if not path.endswith('/'):
	    path = path + '/'
        finalStatDict = dict()
        tcrFout = open(sumF + '.SC_TCRs.txt','a')
	opened = False
        if((os.path.exists(path)) & (os.path.isdir(path))):
            sys.stdout.write(str(datetime.datetime.now()) + " Working on single-cell folder: " + path + '\n')
            sys.stdout.flush()
            (found, nbam, nunmapped, noutput) = formatFiles(path, bam, unmapped, output)
            if not found:
                sys.stderr.write(str(datetime.datetime.now()) + " No bam or unmapped file found in this single-cell folder, please check folder path or data source\n")
                sys.stderr.flush()
            else:
                ...
                opened = addCellToTCRsum(path, noutput, opened, tcrFout)
                finalStatDict = addToStatDict(noutput, path, finalStatDict)
    else:
        if path == './':
            path = os.getcwd()
        if not path.endswith('/'):
    ...
    sumFout = open(sumF + '.summary.txt','a')
